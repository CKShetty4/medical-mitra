import Chat from "./Chat.png"
import Close from "./Close.svg"
import corner from "./corner.svg"
import Home from "./Home.svg"
import Insta from "./instagram.png"
import Linkdin from "./linkedin.png"
import Login from "./Login.svg"
import Logo from "./Logo.svg"
import Mail from "./email.png"
import MedicalMitraLogo from "./MedicalMitraLogo.svg"
import WhatsappLogo from "./WhatsappLogo.svg"
import logo from "./Logo.png"
import vision from "./vision.png"
import mission from "./mission.png"
import Values from "./Values.png"
import Doctor from "./Doctor.png"
import TextBG from "./TextBG.png"
import Picture1 from "./Picture1.png"
import Youtube from "./youtube.png"
import Facebook from "./facebook.png"
import deadline from "./deadline.png"
import contract from "./contract.png"
import contactlist from "./contact-list.png"
import tracking from "./tracking.png"
import lecture from "./lecture.png"
import degree from "./degree.png"
import medicalschool from "./medical-school.png"
import worldwide from "./worldwide.png"
import counseling from "./counseling.png"
import Contacting from "./Contacting.png"
import message from "./message.png"
import telephone from "./telephone-call.png"
import ContactService from "./ContactService.png"
import SignUP from "./SignUP.jpg"
import bg1 from "./bg1.png"
import bg2 from "./bg2.png"
import bg3 from "./bg3.png"
import bg4 from "./bg4.png"
import bg5 from "./bg5.png"
import bg6 from "./bg6.png"
import bg7 from "./bg7.png"
import bg8 from "./bg8.png"
import bg9 from "./bg9.png"
import bg10 from "./bg10.png"
import bg11 from "./bg11.png"
import bg12 from "./bg12.png"
import schedule from "./schedule.png"
import menuIcon from "./menuIcon.png"
export {Chat,lecture,SignUP,message,ContactService,telephone,Contacting,degree,medicalschool,worldwide,counseling,deadline,contract,contactlist,tracking,Close,corner,Home,Insta,Linkdin,Login,Logo,Mail,MedicalMitraLogo,WhatsappLogo,logo,vision,mission,Values,Doctor,TextBG,Picture1,Youtube,Facebook,bg1,bg2,bg3,bg4,bg5,bg6,bg7,bg8,bg9,bg10,bg11,bg12,schedule,menuIcon}